...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'alpha' ) ) {
		...
		if ( !isPositive( opts.alpha ) ) {
			return new TypeError( 'invalid option. `alpha` parameter must be a positive number. Option: `' + opts.alpha + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'beta' ) ) {
		opts.beta = options.beta;
		if ( !isPositive( opts.beta ) ) {
			return new TypeError( 'invalid option. `beta` parameter must be a positive number. Option: `' + opts.beta + '`.' );
		}
	}
